...
def calculate_compound_interest(money, moneyperyear, percentage, timesperyear):
    ...
    for _ in range(20):
        ...
        rows.append([year, f"{"{:,}".format(round(totalamount, 3))} {currency}"])
        year += 1
    return rows
...
style.theme_use("clam")
style.configure("Treeview", rowheight=25, background="grey", foreground="white", fieldbackground="grey", borderwidth=1, relief="solid")
...
table = ttk.Treeview(table_frame, columns=("Year", "Money"), show="headings", height=20)
